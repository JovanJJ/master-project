...
export default function GeneralSettings({ session, status, profileData, reloadProfile, lang }) {
    ...
    const current = [
        [lang === 'en' ? 'First Name' : "Ime", "firstName", profileData.firstName],
        [lang === 'en' ? 'Last Name' : "Prezime", "lastName", profileData.lastName],
        [lang === 'en' ? 'Profession' : "Profesija", "profession", profileData.profession],
        [lang === 'en' ? 'Location' : "Lokacija", "location", profileData.location],
        [lang === 'en' ? 'Phone' : "Telefon", "phone", profileData.phone],
    ];
            ...
            {current.map(([label, field, value]) => (
                <div key={field} className="flex gap-4 space-y-4 items-center justify-between">
                    ...
                    <button
                        ...
                        type="button"
                    >
                        {editingField === field ? (lang === 'en' ? 'cancel' : "poništi") : (lang === 'en' ? 'edit' : "izmeni")}
                    </button>
                </div>

            ))}
            <div className="flex flex-col pt-4">
                <span>{lang === 'en' ? 'Description:' : 'Opis:'}</span>
                <textarea
                    onFocus={() => setClickDescription(true)}

                    defaultValue={profileData.description && profileData.description}
                    placeholder={!profileData.description ? (lang === 'en' ? "Add a profile description and show what sets you apart! Briefly tell your experience, skills and what you are best at in your craft. A good description helps clients find you easier, gain trust and choose you specifically." : "Dodaj opis profila i pokaži šta te izdvaja! Ispričaj ukratko svoje iskustvo, veštine i ono u čemu si najbolji u svom zanatu. Dobar opis pomaže da te klijenti lakše pronađu, steknu poverenje i izaberu baš tebe.") : ""}
