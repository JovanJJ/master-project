...
export default function Auth({ session, status, profileData, lang }) {
    ...
    if (state.success) {
        return (
            <div className="space-y-4">
                ...
                >
                    {lang === 'en' ? 'Log in again' : 'Ulogujte se ponovo'}
                </button>
            </div>
        );
    }
    ...
    return (
        <form action={formAction}>
            ...
            {status === "loading" ? (
                <div>{lang === 'en' ? 'Loading...' : 'Učitavanje...'}</div>
            ...
            {!passwordChange ? (
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    <label className="w-24 font-medium">{lang === 'en' ? 'password' : 'lozinka'}</label>
                    <input
                        placeholder={"*********"}
                        className="w-full sm:max-w-36 border border-blue-300 rounded p-2 focus:outline-none focus:border-blue-600 transition"
                        readOnly
                ...
                (
                    <div className="flex flex-col pt-7 border-t border-blue-200 gap-2">
                        <div className="flex items-center gap-4">
                            <label className="w-35 font-medium">{lang === 'en' ? 'Current Password' : 'Trenutna lozinka'}</label>
                            <input
                        ...
                        <div className="flex items-center gap-4">
                            <label className="w-35 font-medium">{lang === 'en' ? 'New Password' : 'Nova lozinka'}</label>
                            <input
                                name="newPassword"
                                type="password"
                                className="w-full sm:max-w-36 border border-blue-300 rounded p-2 focus:outline-none focus:border-blue-600 transition"
                        ...
                        <div className="flex items-center gap-4">
                            <label className="w-35 font-medium">{lang === 'en' ? 'Confirm New Password' : 'Potvdi novu lozinku'}</label>
                            <input
                        ...
                        <div className="flex items-center pt-2">
                            ...
                            <div className="w-full sm:max-w-[205px] flex justify-end ">
                                <button type="submit" className="px-3 py-1 rounded-3xl text-white bg-blue-400 cursor-pointer">{lang === 'en' ? 'Change' : 'Izmeni'}</button>
                            </div>
                        </div>
